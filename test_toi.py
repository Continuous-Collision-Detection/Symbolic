...
def main():
    ...
    results = session.evaluate(Global.roots(query, "roots.mx"))
    print(results)
    assert("True" in results)

    # results = rules_to_dict(session.evaluate(Global.compareToI("roots.mx", 0.49)))
    # results = rules_to_dict(session.evaluate(Global.compareToI("roots.mx", 0.243923)))
    results = rules_to_dict(session.evaluate(Global.compareToI("roots.mx", 0)))
    print(results)
